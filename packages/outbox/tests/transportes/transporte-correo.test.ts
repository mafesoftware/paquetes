...
describe("transporteCorreo", () => {
  it("arma el envío con render() y pasa para/asunto/html/texto/de a enviar()", async () => {
  ...
  it("si render() tira, la promesa del Transporte rechaza (no hay try/catch propio, ver JSDoc)", async () => {
    const enviar = vi.fn();
    ...
    await expect(transporte(MENSAJE)).rejects.toThrow("plantilla desconocida");
    expect(enviar).not.toHaveBeenCalled();
  });

  it("si enviar() tira, la promesa del Transporte rechaza", async () => {
